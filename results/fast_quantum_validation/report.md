...
运行短时一维热传导量子GPU、torch.supa和自定义SUPA核验证
...
- 分段耗时：`{'host_to_device_and_sync': 0.0006086952053010464, 'device_compute_and_sync': 0.8967900546267629, 'device_to_host': 0.00013129692524671555, 'total': 0.8977137859910727}`
...
- 进程耗时：`{'subprocess_total': 0.019861587788909674}`
...
```bash
python3 -m thermal_pde_audit.cli run --input examples/minimal_heat.json --output results/fast_quantum_validation --device gpu --quantum-steps 2 --ancilla-qubits 8 --auxiliary-range 16.0 --recovery-point 1 --compare-cpu-gpu --validated-profile --supa-audit --custom-supa-audit --error-decomposition
```
