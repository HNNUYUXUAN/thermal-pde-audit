...
模拟长度10毫米、热扩散率1e-6平方米每秒、初始温升100K的一维热传导，计算0.1秒，使用32个空间点和50个时间步，使用GPU
...
- 白名单执行计划：`{'compare_cpu_gpu': True, 'validated_profile': True, 'supa_audit': True, 'custom_supa_audit': True, 'error_decomposition': True, 'report_level': 'standard', 'sources': {'compare_cpu_gpu': 'cli_override:true', 'validated_profile': 'safe_default:exact_empirical_profile', 'supa_audit': 'derived:custom_supa_requires_supa', 'custom_supa_audit': 'cli_override:true', 'error_decomposition': 'cli_override:true', 'report_level': 'default:standard'}}`
...
- 分段耗时：`{'host_to_device_and_sync': 0.000643756240606308, 'device_compute_and_sync': 0.8908773250877857, 'device_to_host': 0.00013717031106352806, 'total': 0.8918766849674284}`
...
- 进程耗时：`{'subprocess_total': 0.023889731615781784}`
- 实现说明：The custom kernel is optimized for fields of up to 256 values using one 256-thread SUPA block and shared-memory tree reduction.
- 实现说明：The kernel accumulates in float32. Independent float32 and source-float64 checks use separate tolerances.

## 性能

- 量子主后端总调用：`27.972228` s
- CPU 参考总调用：`2.4845622` s
...
```bash
python3 -m thermal_pde_audit.cli run --input /workspace/thermal-pde-audit-skill-validation-20260729/thermal-pde-audit/results/skill_entry_gpu_validation/input.json --output /workspace/thermal-pde-audit-skill-validation-20260729/thermal-pde-audit/results/skill_entry_gpu_validation --device gpu --quantum-steps 2 --ancilla-qubits 8 --auxiliary-range 16.0 --recovery-point 1 --compare-cpu-gpu --validated-profile --supa-audit --custom-supa-audit --error-decomposition
```
